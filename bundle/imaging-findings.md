...
This page describes every physical column in `imaging_findings_anon.parquet`.
...
columns inspected by Q007. For comma-delimited fields, the observed domain is
compositional: decode each nonempty token separately and do not treat every
serialized combination as a new clinical category.
...
| `imaging_findings_anon.linked_study_flag` | Optional string; binary code | Flag identifying a linked exam. Observed `N` 149,892 and `Y` 21,486; 0 nulls. | **Schema; Legend; Observed Q007.** The legend describes the flag but supplies no code meanings; interpreting `N/Y` as no/yes is conventional but remains **Inference**. |
...
| `imaging_findings_anon.__index_level_0__` | Optional `int64`; serialized pandas index | Parquet pandas metadata declares this field as an index; there is no released legend entry and no registered imaging value probe. | **Schema.** Treat the export index as non-clinical and not as a portable natural key. Whether its stored imaging values retain any source-row relationship was not tested by Q007–Q009. |
...
| `imaging_findings_anon.location` | Optional string; compositional nominal code | One or more comma-delimited clock/quadrant/location tokens; 117,742 nulls and 273 distinct serialized combinations. All observed nonempty tokens are legend-listed. | **Schema; Legend; Observed Q007.** Some values have a trailing delimiter; discard only the empty terminal token. See vocabulary below. |
...
| `imaging_findings_anon.changed` | Optional string; compositional nominal code | One or more change tokens; 149,231 nulls and 27 serialized combinations. All observed tokens are legend-listed. | **Schema; Legend; Observed Q007.** Decode comma-separated tokens independently; meanings appear below. |
| `imaging_findings_anon.secondaryfindings` | Optional string; compositional nominal code | Ultrasound secondary findings; 171,349 nulls and 7 serialized combinations using `D,E,L,M,S`. | **Schema; Legend; Observed Q007.** `I` and `P` exist in the legend but were not observed. Decode combinations token by token. |
...
| `imaging_findings_anon.recc` | Optional string; compositional recommendation code | One or more recommendation tokens; 35,632 nulls and 229 serialized combinations. Most tokens are legend-listed. | **Schema; Legend; Observed Q007.** Non-legend tokens occur: `?` in 2 rows, `G` 32, `MC` 588, `MS` 5, and `MT` 59. Do not silently map them. Recommendations may encode follow-up or intervention after assessment and require a temporal-leakage policy for prediction use. |
...
| `imaging_findings_anon.calcfind` | Optional string; compositional nominal code | Calcification morphology/finding; 163,136 nulls and 166 serialized combinations. | **Schema; Legend; Observed Q007.** All observed tokens are legend-listed except `N`, occurring alone in 8 rows. |
...
| `imaging_findings_anon.otherfind` | Optional string; compositional nominal code | Other mammographic finding; 158,244 nulls and 77 serialized combinations. All observed tokens are legend-listed. | **Schema; Legend; Observed Q007.** Decode comma-separated tokens separately. |
| `imaging_findings_anon.implanfind` | Optional string; compositional nominal code | Implant finding; 171,258 nulls and 11 serialized combinations. All observed tokens are legend-listed. | **Schema; Legend; Observed Q007.** Decode comma-separated tokens separately. |
| `imaging_findings_anon.consistent` | Optional string; compositional nominal code | “Consistent with” interpretation; 170,142 nulls and 25 serialized combinations. | **Schema; Legend; Observed Q007.** Non-legend token `B` occurs in 838 rows; do not equate it with legend code `N` (“benign asymmetry”). |
...
| Physical column | Representation | Meaning and observed state | Evidence and caveats |
| --- | --- | --- | --- |
| `imaging_findings_anon.USFinding` | Optional string; compositional nominal code | Ultrasound finding; 147,210 nulls and 57 serialized combinations. All observed tokens are legend-listed. | **Schema; Legend; Observed Q007.** Decode comma-separated tokens independently. |
| `imaging_findings_anon.shape` | Optional string; compositional nominal code | `I` irregular, `O` oval, `R` round. Observed `I` 2,244, `O` 6,680, `R` 425, `R,O` 2; 162,027 nulls. | **Schema; Legend; Observed Q007.** One combination demonstrates compositional use. |
...
| `imaging_findings_anon.margins` | Optional string; compositional nominal code | Ultrasound margin codes; 163,976 nulls and 22 serialized combinations using only `A,C,I,M,N,P,R,S`. | **Schema; Legend; Observed Q007.** Decode combinations separately. |
...
| `imaging_findings_anon.echotexture` | Optional string; compositional nominal code | `A` anechoic, `I` isoechoic, `M` mixed hyperechoic/hypoechoic, `P` hypoechoic, `R` hyperechoic. There are 164,270 nulls and 13 serialized combinations. | **Schema; Legend; Observed Q007.** |
| `imaging_findings_anon.posteriorfeatures` | Optional string; compositional nominal code | Posterior features; 169,644 nulls and 8 serialized combinations using only `C,E,G,N,P,T`. | **Schema; Legend; Observed Q007.** |
| `imaging_findings_anon.vascularity` | Optional string; compositional nominal code | Vascularity; 168,687 nulls and 6 serialized combinations using only `A,H,L,M,R`. | **Schema; Legend; Observed Q007.** |
| `imaging_findings_anon.surroundingtissue` | Optional string; compositional nominal code | Surrounding tissue; 170,731 nulls and 6 serialized combinations using only `D,E,F,U`. | **Schema; Legend; Observed Q007.** |
...
| `imaging_findings_anon.massoc` | Optional string; compositional nominal code | Associated MRI finding; 171,222 nulls and 20 serialized combinations. All observed tokens are legend-listed. | **Schema; Legend case-only match; Observed Q007.** Decode combinations independently. |
| `imaging_findings_anon.mother` | Optional string; compositional nominal code | No legend header. Observed 14 serialized domains using tokens `B,C,G,H,I,L,M,N,O,P,R,Y`; 170,438 nulls. | **Schema; Observed Q007; Unresolved.** Do not borrow meanings from another field solely because tokens overlap. |
...
- `size`, `distance`, `numfind`, `calcnumber`, and `msize` need explicit
  ...
  legend units.
- Comma-bearing fields are compositional. Validate tokens against the
  field-specific vocabulary; do not apply a vocabulary from another field.
...
[the investigation results](../docs/feature-context-investigation-results.md).
