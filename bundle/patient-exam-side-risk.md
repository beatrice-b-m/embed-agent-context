...
| `exam_level_anon` | 131,052 | `exam_level_anon.acc_anon` is complete and unique in this release. Treat the table as one row per represented accession, without assuming a cross-release database constraint. **Cross-table check (Q009).** |
...
| `patients_anon.PATIENT_BIRTH_DT_anon` — `timestamp[ns]`, anonymized date-like value | Name suggests an anonymized patient birth date. | Footer metadata reports 81 nulls; shifting method and preserved interval/order properties were not tested. | Representation/missingness: **Release schema.** Meaning from name: **Inference.** The field is absent from the legend; date behavior is **Unresolved**. Never emit values. |
...
| Feature and representation | Meaning or observed domain | Missing and sentinel evidence | Evidence and caveats |
| --- | --- | --- | --- |
| `exam_level_anon.empi_anon` — `int64`, identifier | Unique patient identifier. | No nulls by footer metadata; cardinality was not profiled in this table. | Meaning: **Release legend.** Completeness: **Release schema metadata.** Never emit values. |
| `exam_level_anon.studydate_anon` — `timestamp[ns]`, anonymized date | Exam date. | No nulls by footer metadata; anonymization behavior was not tested. | Meaning: **Release legend.** Representation/completeness: **Release schema.** Preserved ordering/interval properties are **Unresolved**; never emit values. |
| `exam_level_anon.acc_anon` — `int64`, identifier | Anonymized exam record identifier. | No nulls; 131,052 distinct values in 131,052 rows. | Completeness/uniqueness: **Cross-table check (Q009).** The legend describes `acc` and `accession`, not this exact header, as a unique site record identifier for an exam; mapping is **Inference.** Never emit values. |
...
| `exam_level_anon.loc_num_anon` — string, nominal | Name suggests anonymized exam location. The base legend header `loc_num` lists `LOC001`–`LOC003` and `LOC005`–`LOC014` as Sites 1–3 and 5–14; `LOC004` is not listed. | Footer metadata reports no nulls; observed values were not read. | Representation/missingness: **Release schema.** Base meaning/codes: **Release legend.** Applicability to the suffixed V2 field: **Inference; Unresolved.** |
...
| `exam_level_anon.tissueden` — `int8`, nominal code with an ordered 1–4 subset | Observed: `1` almost entirely fat 9,183; `2` scattered fibroglandular densities 42,094; `3` heterogeneously dense 43,623; `4` extremely dense 5,600. The legend also lists unobserved `5`, “Normal male.” | 30,552 nulls. | Meanings: **Release legend.** Counts: **Observed V2 values.** Do not place code 5 after 4 on an ordinal scale, and do not collapse null with a code. |
...
| `exam_level_anon.menopauseage_anon` — `int16`, numeric/zero-heavy | Observed range 0–76; name suggests age at menopause. | No nulls; zero occurs 79,792 times and is the only nonpositive value. | Range/counts: **Observed V2 values.** Unit and whether zero means unknown, not applicable, or not yet menopausal are **Unresolved**. |
| `exam_level_anon.pregnancyage_anon` — `int16`, numeric/zero-heavy | Observed range 0–57; the precise pregnancy event represented by the name is not defined. | No nulls; zero occurs 59,543 times and is the only nonpositive value. | Range/counts: **Observed V2 values.** Meaning, unit, and zero semantics are **Unresolved**. |
| `exam_level_anon.menarcheage_anon` — `int16`, numeric/zero-heavy | Observed range 0–55; name suggests age at menarche. | No nulls; zero occurs 34,415 times and is the only nonpositive value. | Range/counts: **Observed V2 values.** Unit and zero semantics are **Unresolved**. |
| `exam_level_anon.proc_flag` — string, one-sided flag | `Y` occurs 7,470 times. The flag identifies exams related to any procedure and is derived from exam description. | 123,582 nulls; no explicit `N` values. | Meaning: **Release legend.** Domain/counts: **Observed V2 values.** Do not interpret null as “No.” Procedure timing and task eligibility are **Unresolved**. |
| `exam_level_anon.biopsy_flag` — string, one-sided flag | `Y` occurs 4,170 times. The flag identifies exams related to a biopsy procedure and is derived from exam description. | 126,882 nulls; no explicit `N` values. | Meaning: **Release legend.** Domain/counts: **Observed V2 values.** Do not interpret null as “No.” Biopsy timing and task eligibility are **Unresolved**. |
...
| `exam_level_anon.path_severity_exam_level` — `int8`, coded aggregate | Observed codes/counts: 0=1,262; 1=466; 2=727; 3=50; 4=2,868; 5=67. | 125,612 nulls. | **Observed V2 values.** No exact or base legend definition exists. Code meanings, aggregation, and temporal availability relative to imaging are **Unresolved**. |
...
| `side_level_anon.path_severity_side_level` — `int8`, coded aggregate | 0=1,293; 1=490; 2=768; 3=55; 4=3,036; 5=69. | 154,228 nulls. | **Observed V2 values.** Code meanings, aggregation, and temporal availability relative to imaging are **Unresolved**. |
...
| `side_level_anon.asymmetry_side_level` — `int8`, count-like aggregate | 0=140,650; 1=11,070; 2=701; 3=42; 4=9; 5=1. | 7,466 nulls, exactly coincident with null `side`. | **Observed V2 values.** Values above 1 conflict with transferring the base “presence” definition; construction is **Unresolved**. |
| `side_level_anon.arch_distortion_side_level` — `int8`, count-like aggregate | 0=151,262; 1=1,128; 2=76; 3=5; 4=2. | 7,466 nulls, exactly coincident with null `side`. | **Observed V2 values.** Values above 1 conflict with transferring the base “presence” definition; construction is **Unresolved**. |
| `side_level_anon.calc_side_level` — `int8`, count-like aggregate | 0=144,557; 1=7,321; 2=519; 3=61; 4=11; 5=2; 6=1; 11=1. | 7,466 nulls, exactly coincident with null `side`. | **Observed V2 values.** Values above 1 conflict with transferring the base “presence” definition; construction is **Unresolved**. |
...
| `risk_anon.NCILIFE` — `double`, numeric output | Range -35–100; q01/q50/q99 = -35/8.5/100. | Null 1,927. `-35` occurs 1,057 times, `-2` 1,673 times, and `100` 9,237 times. | Distribution: **Observed V2 values.** Model/output classification from the name is **Inference**; definition is **Unresolved**. Negative values are sentinel-like; meanings and whether 100 is a cap, sentinel, or valid value need confirmation. |
| `risk_anon.IBISLIFE` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.244/10.9/100. | Null 21,054; `100` occurs 7,460 times. | **Observed V2 values; Inference** that the name denotes a model output; **Unresolved** definition and 100 semantics. |
| `risk_anon.IBISBRCA1` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.1/0.1/1.9. | Null 17,748; `100` occurs once. | **Observed V2 values; Inference** that the name denotes a model output. The field name alone does not establish the modeled outcome or horizon; **Unresolved**. |
| `risk_anon.IBISBRCA2` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.1/0.1/2.1. | Null 17,748; `100` occurs 4 times. | **Observed V2 values; Inference** that the name denotes a model output; **Unresolved** definition and extreme-value semantics. |
| `risk_anon.IBIS10` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.2/3.7/100. | Null 17,748; `100` occurs 8,128 times. | **Observed V2 values; Inference** that the name denotes a model output. A ten-year horizon is suggested only by the name; definition and 100 semantics are **Unresolved**. |
| `risk_anon.IBISPOP10` — `double`, numeric output | Range 0.1–4.0; q01/q50/q99 = 0.1/3.4/4.0. | Null 17,748. | **Observed V2 values; Inference** that the name denotes a model output. Population comparator and horizon are suggested only by the name; **Unresolved**. |
| `risk_anon.IBISPOPL` — `double`, numeric output | Range 0.1–13.4; q01/q50/q99 = 0.1/9.0/13.2. | Null 17,748. | **Observed V2 values; Inference** that the name denotes a model output. Definition is **Unresolved**. |
| `risk_anon.NCI5` — `double`, numeric output | Range -35–100; q01/q50/q99 = -35/1.6/100. | Null 19,323. `-35` occurs 709 times, `-2` 1,673 times, and `100` 6,838 times. | **Observed V2 values; Inference** that the name denotes a model output. A five-year horizon is suggested only by the name. Sentinel meanings and 100 semantics are **Unresolved**. |
| `risk_anon.IBIS_TD1` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.2/7.7/100. | Null 21,136; `100` occurs 7,446 times. | **Observed V2 values; Inference** that the name denotes a model output. `TD1` meaning is **Unresolved**; do not assume equivalence to tissue-density code 1. |
| `risk_anon.IBIS_TD2` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.2/9.7/100. | Null 21,136; `100` occurs 7,446 times. | **Observed V2 values; Inference** that the name denotes a model output. `TD2` meaning is **Unresolved**. |
| `risk_anon.IBIS_TD3` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.3/11.5/100. | Null 21,136; `100` occurs 7,446 times. | **Observed V2 values; Inference** that the name denotes a model output. `TD3` meaning is **Unresolved**. |
| `risk_anon.IBIS_TD4` — `double`, numeric output | Range 0.1–100; q01/q50/q99 = 0.4/15.3/100. | Null 21,136; `100` occurs 7,446 times. | **Observed V2 values; Inference** that the name denotes a model output. `TD4` meaning is **Unresolved**. |
...
The physical schema contains 12 NCI/IBIS-named numeric fields.
`risk_anon.__index_level_0__` is a separate pandas index field, not an
additional output. **Release schema.**
...
8. Is `exam_level_anon.acc_anon` intended as the canonical exam key, and is
   its observed completeness/uniqueness guaranteed across releases?
